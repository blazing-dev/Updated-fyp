...
# Headers for recognition and counting
from FootCountAndRecognition.driver_script import DriverForCamera1
from FootCountAndRecognition.driver_script import DriverForCamera2
...
inst = DriverForCamera1()
def get_frame1():
    for _, _, _, frameEntry in inst.StartEntryCountAndRecognition():
        imgencode=cv2.imencode('.jpg', frameEntry)[1]
        stringData=imgencode.tostring()
        yield (b'--frame\r\n'b'Content-Type: text/plain\r\n\r\n'+stringData+b'\r\n')
        del(frameEntry)

inst1 = DriverForCamera2()
def get_frame2():
    for _, _, _, frameExit in inst1.StartExitCountAndRecognition():
        imgencode=cv2.imencode('.jpg', frameExit)[1]
        stringData=imgencode.tostring()
        yield (b'--frame\r\n'b'Content-Type: text/plain\r\n\r\n'+stringData+b'\r\n')
        del(frameExit)

def liveEntryExit(request, stream_path = "video", stream_path1 = "video1"):
    if stream_path == "video":
        try :
            return StreamingHttpResponse(get_frame1(),content_type="multipart/x-mixed-replace;boundary=frame")
        except:
            return "error"
    elif stream_path1 == "video1":
        try :
